...
> **REVISED 2026-07-20:** (a) **YouTube export (4.3) is DEFERRED** — owner questioned its value; the share sheet ships with Copy-link + `navigator.share` + Spotify/Apple **search** deep links only, and the YouTube row is a later opt-in. (b) **"Build for your city" (4.4)** is partly superseded by auto-location (middleware already routes a fresh visitor to their city) — its remaining job is a masthead button that reaches the picker via `/?pick=1` (not re-triggering the auto-redirect), so keep it but re-point it. (c) OG cards' "top 3 finds" = the opener-forward billing picks. (d) Ticket links come from JamBase `offers[]` (seller e.g. DICE), not TM.
